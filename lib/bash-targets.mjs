...
export function detectWriteTargets(command, repoRoot, cwd = null) {
  ...
  // under the repo root as before. Resolve the CWD through symlinks first so it
  // lands in the SAME namespace as the git-canonical root — otherwise a symlinked
  // cwd (e.g. macOS $TMPDIR /var → /private/var) makes every relative target look
  // "outside the repo" and the claim is silently skipped.
  let base = (cwd || root).replace(/\\/g, "/").replace(/\/$/, "");
  try {
    base = realpathSync(base).replace(/\\/g, "/").replace(/\/$/, "");
  } catch {
    /* cwd may not exist (or be unreadable) — fall back to the raw path */
  }
  const found = new Set();
  ...
  const add = (raw) => {
    ...
    const abs = isAbsolute(t) ? t.replace(/\\/g, "/") : resolve(base, t).replace(/\\/g, "/");
