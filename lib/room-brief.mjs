...
import { issueStats, coordToolIssues } from "./issues.mjs";
import { DEAD_MS, HOOK_CONTEXT_BUDGET } from "./config.mjs";
import { clampLines } from "./budget.mjs";

// The workspace of the repo THIS code lives in — i.e. the agent-coord checkout
// itself. A session working here is the one place cross-repo coord-tool issue
// reports should surface (see coordToolIssues).
const HOME_WS = (() => {
  try {
    return wsOf(join(dirname(fileURLToPath(import.meta.url)), ".."));
  } catch {
    return null;
  }
})();
...
export function buildRoomBrief(db, { agentId, workspaceId, repoRoot, budget = HOOK_CONTEXT_BUDGET, wrap = (s) => s }) {
  const lines = [];
  ...
  // Self-triage: a session in the agent-coord repo itself gets told about open
  // issues that mention the coordination tool but were filed from OTHER repos —
  // otherwise the workspace scoping hides field reports from the only repo that
  // can fix them (they sat unread for three weeks once).
  if (HOME_WS && workspaceId === HOME_WS) {
    const ct = coordToolIssues(db, { excludeWorkspaceId: workspaceId, limit: 3 });
    if (ct.total) {
      lines.push(`  🔧 ${ct.total} open issue${ct.total > 1 ? "s" : ""} about agent-coord filed from other repos — survey: cli/issues.mjs`);
      for (const i of ct.issues) lines.push(`     ○ [${i.severity}] ${String(i.title).slice(0, 80)} (${i.issue_id}, ${i.project})`);
    }
  }

  // Waiting mail. One string serves both channels (SessionStart stdout for
  // Claude, announce_intent JSON for Codex), so the wording must not steer a
  // hook-fed agent into a redundant read_messages call — Claude gets these
  // injected automatically; pulling is the fallback for agents that don't.
  const unread = unreadCount(db, { agentId, workspaceId });
  if (unread) lines.push(`  ✉ ${unread} unread message${unread > 1 ? "s" : ""} — they reach you automatically if you're hook-wired (Claude); otherwise read_messages to pull them`);
  ...
  // SessionStart stdout hits the same 8192-byte cliff as mid-turn delivery: past
  // it, Claude Code files the brief away and shows a 2000-char preview instead.
  // Every slice above is already bounded, so this is a backstop rather than the
  // plan — but the identity line must survive whatever else goes, and a trim has
  // to announce itself. Nothing here is consumed on read, so a dropped line is
  // deferred, not lost.
  return clampLines([head, ...lines], {
    budget,
    wrap,
    keep: 1,
    note: (n) => `  ⋯ ${n} line(s) trimmed to fit the session-context limit — list_active_agents / list_tasks / list_decisions pull the rest`,
  }).join("\n");
}
