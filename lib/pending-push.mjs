...
export function analyzePendingPush(db, repo, meAgentId = null) {
  const ws = workspaceId(repo);
  let ahead;
  try {
    ahead = git(["log", "@{u}..HEAD", "--format=%h%x09%s"], repo);
  } catch {
    return { upstream: false, commits: [], recommendation: "No upstream tracking branch — can't tell what's unpushed." };
  }
  const lines = ahead.split(/\r?\n/).filter(Boolean);
  if (!lines.length) return { upstream: true, commits: [], recommendation: "Up to date with upstream — nothing to push." };
  ...
  // caller (the MCP server) may run under a DIFFERENT id: it resolves identity once
  // at startup and, if it lost the SessionStart race or the session resumed, falls
  // back to a standalone id and never reconciles — so its own session's commits look
  // like a live peer's. Bridge that here by also treating the session-linked id for
  // our parent process as ours. Read-only and additive: it can only ever recognize
  // OUR OWN hook identity, never clear a genuine peer's commit.
  const myIds = new Set([meAgentId, readSessionLink(process.ppid)].filter(Boolean));
  ...
  return { upstream: true, commits, recommendation: parts.join("; "), allClear, blockers: blockers.map((c) => ({ hash: c.hash, agent: c.agent, verdict: c.verdict })) };
}
