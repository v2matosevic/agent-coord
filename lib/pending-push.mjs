...
export function analyzePendingPush(db, repo, meAgentId = null, { ppids = null } = {}) {
  ...
  const linked = ppids && ppids.length ? readSessionLinkAny(ppids) : readSessionLink(process.ppid);
  const myIds = new Set([meAgentId, linked].filter(Boolean));
