...
export function analyzePendingPush(db, repo, meAgentId = null, { ppids = null, includeAmbientIdentity = true } = {}) {
  ...
  const provenance = new Map(
    // Explicit sequence order makes the latest attribution win, independently
    // of SQLite's chosen index or equal timestamps.
    db.prepare("SELECT agent_id, detail FROM activity_log WHERE event='commit' AND workspace_id=? ORDER BY seq ASC").all(ws).map((r) => [String(r.detail).split("\t")[0], r.agent_id]),
  );
