...
CREATE TRIGGER IF NOT EXISTS fts_task_ad AFTER DELETE ON tasks BEGIN
  DELETE FROM search_index WHERE kind='task' AND ref=old.task_id;
END;
CREATE TRIGGER IF NOT EXISTS fts_issue_ai AFTER INSERT ON issues BEGIN
  INSERT INTO search_index(text, kind, ref, workspace_id, agent, ts)
  VALUES (new.title || COALESCE(' — ' || new.body, '') || COALESCE(' — ' || new.area, '') || COALESCE(' — ' || new.resolution, ''),
          'issue', new.issue_id, new.workspace_id, new.agent_id, new.ts);
END;
CREATE TRIGGER IF NOT EXISTS fts_issue_au AFTER UPDATE ON issues BEGIN
  DELETE FROM search_index WHERE kind='issue' AND ref=old.issue_id;
  INSERT INTO search_index(text, kind, ref, workspace_id, agent, ts)
  VALUES (new.title || COALESCE(' — ' || new.body, '') || COALESCE(' — ' || new.area, '') || COALESCE(' — ' || new.resolution, ''),
          'issue', new.issue_id, new.workspace_id, new.agent_id, new.ts);
END;
CREATE TRIGGER IF NOT EXISTS fts_issue_ad AFTER DELETE ON issues BEGIN
  DELETE FROM search_index WHERE kind='issue' AND ref=old.issue_id;
END;
`;
...
INSERT INTO search_index(text, kind, ref, workspace_id, agent, ts)
  SELECT title || COALESCE(' — ' || body, '') || COALESCE(' — ' || area, '') || COALESCE(' — ' || resolution, ''),
         'issue', issue_id, workspace_id, agent_id, ts FROM issues;
`;

// `issues` is the first record kind added to the FTS index AFTER it could already
// exist on a live store (messages/decisions/tasks built it first, were backfilled
// once, and on that store `existed` is true forever). On that warm path the one-shot
// BACKFILL above never runs, so issue rows the triggers didn't witness — every issue
// reported before the first search() of the post-upgrade era — would be invisible to
// search permanently. This idempotent catch-up indexes any issue not already in the
// index; the NOT IN guard makes it safe to run on every first-search and a no-op once
// caught up (and on the cold path, BACKFILL already covered issues, so this finds none).
const ISSUE_CATCHUP = `
INSERT INTO search_index(text, kind, ref, workspace_id, agent, ts)
  SELECT title || COALESCE(' — ' || body, '') || COALESCE(' — ' || area, '') || COALESCE(' — ' || resolution, ''),
         'issue', issue_id, workspace_id, agent_id, ts
  FROM issues WHERE issue_id NOT IN (SELECT ref FROM search_index WHERE kind='issue');
`;
...
export function ensureSearchIndex(db) {
  ...
  try {
    ...
    writeTxn(db, () => {
      ...
      else db.exec(ISSUE_CATCHUP); // warm store: catch up a kind added after the index already existed
    });
    _ftsState = true;
  } catch {
...
const KINDS = new Set(["message", "decision", "task", "issue"]);
...
export function searchRecords(db, { workspaceId, query, kinds = null, limit = 12 }) {
  ...
  if (wanted.includes("issue")) {
    out.push(
      ...db
        .prepare(`SELECT 'issue' kind, issue_id ref, agent_id agent, ts, title || COALESCE(' — ' || body, '') snippet FROM issues WHERE ${likes("title || COALESCE(' — ' || body, '') || COALESCE(' — ' || area, '') || COALESCE(' — ' || resolution, '')")} AND (workspace_id IS NULL OR workspace_id=?) ORDER BY rowid DESC LIMIT ?`)
        .all(...terms, workspaceId, lim),
    );
  }
