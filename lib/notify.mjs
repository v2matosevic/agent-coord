...
function throttled(key) {
  ...
  try {
    // Atomic write (temp + rename) so a concurrent hook never reads a torn file
    // and loses all throttle state. The read race remains — worst case a duplicate
    // banner, never a missed block — which is acceptable for a best-effort alert.
...
function planNotification({ title = "agent-coord", subtitle = "", message = "", key = "", sound = false }) {
  ...
  return { cmd: "/usr/bin/osascript", args: ["-e", script] }; // absolute — GUI/cron PATH may lack it
}
