...
// Liveness writes are throttled through a local marker file: a bare heartbeat
// (no task change) is skipped when the last successful DB heartbeat was under
// this threshold ago. Hooks fire on EVERY tool call fleet-wide, and each bare
// heartbeat is a BEGIN IMMEDIATE write txn on the one shared store — this
// removes that write-lock pressure from the hot path. MUST stay well under
// DEAD_MS (45s = 4x margin): the worst-case staleness of a live agent's
// last_heartbeat is this value, and liveness checks compare against DEAD_MS.
export const HB_THROTTLE_MS = 45 * 1000;
...
// Context-budget caps — the coordination layer writes INTO model context (hook
// stdout, MCP results), so an unbounded list is a token leak: one busy day's
// backlog could dump a wall into every agent's turn. Caps are per-call, never
// lossy — message reads advance their pointer only past what was RETURNED, so
// the remainder arrives on the next call/turn — and every truncation says so
// out loud (no silent "that was everything" illusion).
export const MSG_READ_MAX = 30; // read_messages: max messages per call
export const MSG_DELIVER_MAX = 15; // mid-turn / prompt-time delivery: max messages injected per event
export const STATE_LIST_MAX = 50; // get_global_state: max rows per list (leases/queue/tasks)

// Hook stdout budget. Claude Code files away any hook output over 8192 BYTES and
// shows the model a 2000-char preview instead — silently, with no error and no
// signal to the hook. Since delivery also consumes (the read pointer advances),
// an over-budget write used to destroy every message past the preview. A COUNT
// cap can't bound this: 15 ordinary messages are 12–30 KB. So the caps below are
// in bytes, enforced in lib/budget.mjs + lib/coord-context.mjs, and sit under
// 8192 so the limit can be lowered without this going silent again.
export const HOOK_CONTEXT_BUDGET = Number(process.env.AGENT_COORD_HOOK_BUDGET) || 7600;
// Body widths tried, widest first: shorten every body to a common width before
// withholding anything, so a sender line + first line always survives — that's
// what lets an agent decide whether to call read_messages.
export const MSG_BODY_WIDTHS = [Infinity, 900, 450, 220, 110];
// Directed messages stranded past the MSG_DELIVER_MAX window are pulled forward
// (a yield request must not wait out a broadcast backlog).
export const MSG_DIRECTED_LOOKAHEAD = 5;
