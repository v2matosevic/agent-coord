...
export function readSessionLink(claudePid) {
  return readSessionLinkMeta(claudePid)?.agentId ?? null;
}

// The link with its write time — a reader that must know whether the link is
// NEWER than itself (the MCP server following a /clear rename) needs `ts`.
export function readSessionLinkMeta(claudePid) {
  if (!claudePid) return null;
  try {
    const l = JSON.parse(readFileSync(linkFile(claudePid), "utf8"));
    if (l && l.agentId && Date.now() - l.ts < LINK_TTL_MS) return { agentId: l.agentId, ts: Number(l.ts) || 0 };
  } catch {}
  return null;
}
...
export function sessionAnchorPids(ppid = process.ppid) {
  const pids = [];
  // Claude Code hands its own pid to the children it spawns (`CLAUDE_PID`); when
  // present it IS the anchor, no process walk needed — and it works where the
  // walk can't (a Claude installed via npm runs as plain `node`, so no ancestor
  // is named "claude").
  const envPid = Number(process.env.CLAUDE_PID);
  if (envPid > 0) pids.push(envPid);
  try {
    const cp = findClaudePid(ppid);
    if (cp && !pids.includes(cp)) pids.push(cp);
  } catch {}
  if (!pids.includes(ppid)) pids.push(ppid);
  return pids;
}

// First link (with metadata) found under any candidate anchor.
export function readSessionLinkMetaAny(claudePids) {
  for (const pid of claudePids || []) {
    const m = readSessionLinkMeta(pid);
    if (m) return m;
  }
  return null;
}
