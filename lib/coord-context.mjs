...
import { holderWaitNotices, ackHolderNotices } from "./contention.mjs";
...
export function midTurnContext(db, { agentId, workspaceId: ws, budget = HOOK_CONTEXT_BUDGET, wrap = (s) => s }) {
  ...
  const before = [];
  const after = [];
  const holderNotices = holderWaitNotices(db, { agentId, workspaceId: ws }).map(n => ({ ...n,
    line: `${n.agentId} is waiting for "${headline(n.path, 220)}". Finish the running command and a coherent edit, then release this file at a safe handoff point. Do not unlock during a command.`,
  }));
  for (const n of holderNotices) after.push(n.line);
  const acknowledgeNotices = (text) => {
    const offered = holderNotices.filter(n => text?.includes(n.line));
    const claimed = new Set(ackHolderNotices(db, agentId, offered.map(n => n.waitId)));
    // Concurrent context events can race the initial read. Only the winner of
    // the unique notice insert emits it; removing a line only shrinks the budget.
    const omitted = new Set(offered.filter(n => !claimed.has(n.waitId)).map(n => n.line));
    const result = text?.split("\n").filter(line => !omitted.has(line)).join("\n");
  ...
  if (!picked) {
    ...
    return acknowledgeNotices(text);
  }
  ...
  return acknowledgeNotices(picked.text);
}
