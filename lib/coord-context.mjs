import { readMessages, annotateSenders, unreadCount } from "./messages.mjs";
...
export function midTurnContext(db, { agentId, workspaceId: ws }) {
  ...
  const msgs = annotateSenders(db, readMessages(db, { agentId, workspaceId: ws, limit: MSG_DELIVER_MAX }));
  for (const m of msgs) lines.push(`• ${m.from_agent}${m.from_live ? "" : " (exited — not live)"}${m.to_agent ? " → you" : ""}: ${m.body}`);
  if (msgs.length === MSG_DELIVER_MAX) {
    const more = unreadCount(db, { agentId, workspaceId: ws });
    if (more > 0) lines.push(`…and ${more} more unread — arriving over your next tool calls (or read_messages to pull them now).`);
  }
