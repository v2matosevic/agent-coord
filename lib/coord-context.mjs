...
export function midTurnContext(db, { agentId, workspaceId: ws }) {
  ...
  // 1) Unread peer messages — directed-to-me or workspace/global broadcasts.
  const msgs = readMessages(db, { agentId, workspaceId: ws });
  for (const m of msgs) lines.push(`• ${short(m.from_agent)}${m.to_agent ? " → you" : ""}: ${m.body}`);
  if (msgs.length) {
    // Ping the human too — read-once delivery means this fires exactly once per message.
    const last = msgs[msgs.length - 1];
    notify({
      title: msgs.length > 1 ? `📬 ${msgs.length} new messages` : `📬 ${short(last.from_agent)}`,
      message: msgs.length > 1 ? `${short(last.from_agent)}: ${last.body}` : last.body,
      key: `msg:${ws}`,
      sound: true,
    });
  }

  // 2) Duplicate-work advisory — only to the later-starter, throttled. The
