...
// Open issues ABOUT the coordination tool itself, filed from OTHER repos — the
// one legitimate crack in the workspace-scoping rule: an agent hits a coord bug
// in a client repo and report_issue tags it with THAT workspace, so the
// agent-coord repo (the only place that can fix it) never sees it in its own
// scoped list; the reports sat unread for weeks. Consumed by the room brief,
// gated to sessions IN the agent-coord repo. Filtered to tool-referencing
// issues (name a coord tool/hook, or kind 'coordination') so another client's
// unrelated backlog never bleeds into a session's context.
const COORD_TERMS = [
  "agent-coord",
  "coord",
  "post_message",
...
export function coordToolIssues(db, { excludeWorkspaceId = null, limit = 3 } = {}) {
  const likes = COORD_TERMS.map(() => "(title LIKE ? OR area LIKE ? OR body LIKE ?)").join(" OR ");
  const vals = COORD_TERMS.flatMap((t) => [`%${t}%`, `%${t}%`, `%${t}%`]);
  const rows = db
    .prepare(`SELECT * FROM issues WHERE status='open' AND (workspace_id IS NULL OR workspace_id<>?) AND (kind='coordination' OR ${likes})`)
    .all(excludeWorkspaceId || "", ...vals);
  rows.sort((a, b) => (SEV[b.severity] ?? 1) - (SEV[a.severity] ?? 1) || String(b.ts).localeCompare(String(a.ts)));
  return { total: rows.length, issues: rows.slice(0, Math.max(1, Number(limit) || 3)).map(shape) };
}
